...
> 前置：先把 `docs/api/posts-api.md`（数据接口，含公告 config 与媒体上传）落地。
...
站点侧如需「打开云端镜像版」的入口，作为**可选部署项**处理（代码不预置，由部署方决定是否添加）：

- 实现方式极简：在站点任意位置加一个普通 `<a href="<MIAODA_APP_URL>" target="_blank" rel="noopener">` 链接即可（如导航栏或页脚）。
...
1. ✅ hash 直达（前端已完成）
2. Base 镜像表 + `sync-mirror.js` 定时同步（妙搭和机器人都依赖它）
3. webhook/机器人 + `pushFeatured`（精华即时推送，顺带即时 upsert 镜像）
4. `sendFeaturedDigest` 周报
5. 妙搭应用接 Base 数据源，配置机器人菜单
6. 机器人入群发卡并置顶社区链接（`im/v1/pins`，v2.1）
7. （可选）站点内加「云端版」跳转链接（5.1，部署方决定）
